...
$('#btn-submit').on("click", function() {
    ...
    {
        destinationCity = $("#destination-input").val().trim();
    }
    if( $("#origin-input").val()!=undefined)
    {
        originCity = $("#origin-input").val().trim();
    }
    //TODO: setup tripDate searching
    if($("#time-input").val()!=undefined)
    {
        tripDate = $("#date-input").val().trim();
    }
    else{
    ...
    var directionsURL = "http://www.mapquestapi.com/directions/v2/route?key=Gx9QGTMeo5RatQTBAvX2JHdG9Au9KUkD&from=" +
    ...
    var weatherQueryURL = "https://api.openweathermap.org/data/2.5/weather?q="
                        +destinationCity + "&units=metric&appid=" + weatherApiKey;
    ...
    $.ajax({
      ...
      .then(function(response) {
        ...
        var forecastQueryURL="https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/" + forecastApiKey 
                            + "/" + latitude + "," + longitude;
        $.ajax({
            ...
            .then(function(response) {
                ...
                {
                    //test string
                    var dailyForecast="High: " + forecastDays[i].temperatureHigh 
                                    + " Low: " + forecastDays[i].temperatureLow
                                    + " Summary: " + forecastDays[i].summary + " Icon: " + forecastDays[i].icon;
